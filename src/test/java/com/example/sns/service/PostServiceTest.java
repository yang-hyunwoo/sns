...
public class PostServiceTest {
    ...
    void 포스트수정이_성공한경우() {
        ...
        var postEntity  = PostEntityFixture.get(userName,postId,1);
        var userEntity =postEntity.getUser();
    ...
    void 포스트수정시_권한이_경우() {
        ...
        var postEntity  = PostEntityFixture.get(userName,postId,1);
        var writer = UserEntityFixture.get("userName1", "password",2);

        when(userEntityRepository.findByUserName(userName)).thenReturn(Optional.of(writer));
        when(postEntityRepository.findById(postId)).thenReturn(Optional.of(postEntity));

        SnsApplicationException e = Assertions.assertThrows(SnsApplicationException.class, () -> postService.modify(title, body, userName, postId));
        Assertions.assertEquals(ErrorCode.INVALID_PERMISSON,e.getErrorCode());
    }
